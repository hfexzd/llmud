...
def create_router(
    ...
    async def game_action(request: ActionRequest):
        ...
        # Wreath making at bamboo_forest
        if "编花环" in filtered_input and player.current_scene == "bamboo_forest":
            new_inv = list(player.inventory or []) + ["花环"]
            player = player.model_copy(update={"inventory": new_inv})
            _response_extras[0] = "你采来野花，编了一个漂亮的花环。获得【花环】。"
        ...
        # Pottery at outer_gate
        if "制陶" in filtered_input and player.current_scene == "outer_gate":
            new_inv = list(player.inventory or []) + ["陶罐"]
            player = player.model_copy(update={"inventory": new_inv})
            _response_extras[0] = "你用泥巴捏了一个陶罐，虽然粗糙但还算实用。获得【陶罐】。"
        ...
        # Fortune telling at misty_lake
        if "算命" in filtered_input and player.current_scene == "misty_lake" and player.spirit_stones >= 5:
            import random as _fortune
            fortunes = ["前途无量", "小心水逆", "近日有喜", "宜出行", "宜修炼", "勿近水", "财运亨通", "诸事不宜"]
        ...
        # Stargazing at misty_lake
        if "观星" in filtered_input and player.current_scene == "misty_lake":
            gain = 2
            player = player.model_copy(update={"spirit_power": player.spirit_power + gain})
        ...
        if "品茶" in filtered_input and player.current_scene == "inner_gate":
            ...
            if profile_row:
                ...
                new_stage = compute_relationship_stage(new_fav)
                npc_repo.update_favorability("linwaner", new_fav, new_stage)
                _response_extras[0] = f"你和林婉儿师姐品茶论道，相谈甚欢。（好感度+2）。"
            else:
        ...
        # Trap setting at mountain_range
        if "设陷阱" in filtered_input and player.current_scene == "mountain_range":
            import random as _trap
            if _trap.random() < 0.35:
                new_inv = list(player.inventory or []) + ["野兔"]
                player = player.model_copy(update={"inventory": new_inv})
                _response_extras[0] = "你设下的陷阱捕获了一只野兔！获得【野兔】。"
            else:
                _response_extras[0] = "陷阱空空的，什么也没捕到。"
