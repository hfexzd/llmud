...
13. 【世界增量】你可以在JSON中附带一个`world_delta`字段，用于提议世界状态的微小变化（非强制——无变化则省略该字段）。world_delta的JSON结构为：{{"tension":{{"<张力id>":{{"pressure":<整数>, "progress":{{"<路径id>":<整数>}}}}}}, "npc":{{"<npc_id>":{{"mood":"<心境>", "scene_id":"<场景id>", "goal_progress":{{"<goal_id>":<整数>}}}}}}, "faction":{{"<势力名>":{{"trust":<整数>}}}}}}。其中：
  ...
  - npc_id 必须来自上方【世界设定·可述及事物】中的人物，faction的势力名必须来自其中的势力（目前有：青云门、散修盟）；scene_id 可选，用于将NPC移动到玩家当前所在或邻近的场景（如NPC随行、离去等），必须为系统已有的场景id；【极重要·NPC位置同步】当你叙述NPC与玩家一同前往某处、跟随玩家、离开某地时，你【必须】在world_delta.npc中更新该NPC的scene_id。例如玩家与林婉儿同去竹林，JSON中必须包含{{"world_delta":{{"npc":{{"linwaner":{{"scene_id":"bamboo_forest"}}}}}}}}。漏掉会导致NPC位置与叙事不一致，严重影响游戏体验。
  ...
  - 当NPC在叙事中跟随玩家移动、离开某场景、或心境发生显著变化时，你【必须】在world_delta中记录（更新scene_id/mood等字段），以便系统同步NPC状态；
  - NPC的自主行动（如离去、前往别处、情绪波动）也应通过world_delta体现，使玩家能感知到NPC的独立存在感。
  - world_delta中的一切实体引用同样受规则12约束：不得提及不存在的张力、NPC或势力。
14. 【行动建议·每次必做】你【必须】在每次回复的JSON中附带"suggested_actions"字段，值为2-4个中文短句的字符串数组，根据当前故事内容建议玩家下一步可尝试的行动。例如：故事提到灵草发光且有妖气→建议["细查灵草","探查妖气来源","与林婉儿商议对策"]；故事提到妖兽出没→建议["准备战斗","观察妖兽动向","寻找妖兽弱点"]。即使只是场景描写，也要给出符合当前语境的行动建议。这些会显示在玩家的行动按钮栏中。"""
